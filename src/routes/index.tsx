    ...
    <div className="min-h-screen bg-background text-foreground">
            ...
            <div className="relative overflow-hidden rounded-[2rem] border border-border bg-card shadow-[var(--shadow-elegant)]">
              ...
              <div className="absolute bottom-4 left-4 right-4 flex flex-wrap gap-2">
                <div className="flex items-center gap-2 rounded-full bg-background/90 px-3 py-1.5 text-xs font-semibold shadow-sm backdrop-blur text-foreground border border-border">
                  <span className="h-2 w-2 rounded-full bg-forest animate-ping" /> Recommended: Honey Process
                </div>
                <div className="flex items-center gap-2 rounded-full bg-background/90 px-3 py-1.5 text-xs font-semibold shadow-sm backdrop-blur text-foreground border border-border">
          ...
          <div className="grid gap-12 md:grid-cols-2 md:items-center">
            ...
            <div className="grid gap-6 sm:grid-cols-2">
              ...
              <div className="p-6 rounded-2xl bg-card border border-border shadow-sm">
                ...
                <div className="mt-2 text-xs text-muted-foreground">
                  Keeps green coffee beans perfectly stable and completely mold-resistant during storage.
                </div>
              </div>
              <div className="p-6 rounded-2xl bg-card border border-border shadow-sm">
                ...
                <div className="mt-2 text-xs text-muted-foreground">
                  Aides in targeting optimal pH fermentation stages (&lt;4.0) to lock in ester and fruity profiles.
                </div>
              </div>
              <div className="p-6 rounded-2xl bg-card border border-border shadow-sm">
      ...
      <section id="methods" className="mx-auto max-w-7xl px-6 py-24">
        <div className="mx-auto max-w-2xl text-center">
          ...
          <p className="mt-3 text-muted-foreground">
            Our smart indicator model balances flavor, water usage, and physical risks for each method.
          </p>
        </div>
        <div className="mt-12 grid gap-5 md:grid-cols-2 lg:grid-cols-5">
          {[
            { name: "Washed", color: "var(--chart-4)", flavor: "Clean, bright acidity", water: "5–40 L/kg", risk: "Low drying risk if clean" },
            { name: "Semi Washed", color: "var(--forest)", flavor: "Balanced body, mild acidity", water: "1–10 L/kg", risk: "Moderate drying risk" },
            { name: "Honey", color: "var(--honey)", flavor: "High sweetness, fruity notes", water: "0.5–5 L/kg", risk: "Sticky mucilage mold risk" },
            { name: "Wine Process", color: "var(--chart-5)", flavor: "Boozy, tropical fruit, tropical acidity", water: "Low (&lt;2 L/kg)", risk: "Over-fermentation risk" },
            { name: "Natural", color: "var(--coffee)", flavor: "Intense fruitiness, heavy body", water: "Very Low (&lt;1 L/kg)", risk: "Extreme mold risk if RH &gt;70%" },
          ].map((m) => (
            <div
              key={m.name}
      ...
      <footer className="border-t border-border py-8 bg-card">
        <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-4 px-6 text-sm text-muted-foreground">
          <div className="flex items-center gap-2">
            ...
            <span>© {new Date().getFullYear()} TerraBrew. Bridging coffee agronomy and smallholder sustainability.</span>
          </div>
          <div className="flex gap-6">
            <a href="#" className="hover:text-foreground">Databases</a>
            <a href="#" className="hover:text-foreground">Research Sources</a>
